...
var myNodelist: HTMLCollectionOf<Element> = document.getElementsByTagName("LI");
...
function addCloseButton(li) {
  ...
  span.onclick = function() {
    var div = (this as any).parentElement;
...
//   const currentText = listItem.childNodes[0].textContent.trim();
  ...
  const item: ToDoListItem = {
    name: listItem.childNodes[0].textContent.trim(),
  }
...
function addUpdateListeners() {
  ...
  for (let button of updateButtons) {
    const buttonTS: any = button;
    if (!(buttonTS as any).hasListener) {
        buttonTS.addEventListener('click', handleUpdate);
      buttonTS.hasListener = true;
    }
  }
}
//
...
function newElement() {
    console.log('test');
  var inputValue = (document.getElementById("myInput") as any).value;
  if (inputValue === '') {
  ...
  var textNode = document.createTextNode(newItem.name + ' ');
